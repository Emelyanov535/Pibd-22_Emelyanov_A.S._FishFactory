...
namespace FishFactoryBusinessLogic.BusinessLogics
{
    public class OrderLogic : IOrderLogic
    {
        ...
        private void CheckModel(OrderBindingModel model, bool withParams = true)
        {
            ...
            if (model.CannedId < 0)
            {
                throw new ArgumentNullException("Некорректный идентификатор изделия", nameof(model.CannedId));
            }
            ...
            _logger.LogInformation("Order. OrderId:{Id}.Sum:{ Sum}. PackageId: { PackageId}", model.Id, model.Sum, model.CannedId);
        }
    }
}
